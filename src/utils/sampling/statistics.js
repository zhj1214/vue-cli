...
function init() {
  ...
  if (uni.$localStorage.getCurrentUser()) {
    let user = uni.$localStorage.getCurrentUser();
...
function timeExposure(time) {
  ...
  var arr = statistics.ads.map((item, index) => {
    let j = index <= totalNum % statistics.ads.length ? 1 : 0;
    return {
      indicatorType: 1,
      adId: item.id,
      memberId: statistics.memberId,
...
function statisticsAPI(param) {// wx.$api.appLetSamplingStatistics(param)
}
