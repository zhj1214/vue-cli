 ...
 * @return {
   ...
   export const stampGetDate = (timeNum) => {
	   ...
	   const mixHour = parseInt((timeNum % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
	   const mixMinute = parseInt((timeNum % (1000 * 60 * 60)) / (1000 * 60));
	   const mixSecond = (timeNum % (1000 * 60)) / 1000;
	   return {
		   day: mixDay,
		   time: (mixHour + '').padStart(2, '0') + ':' + (mixMinute + '').padStart(2, '0') + ':' + (mixSecond + '').padStart(2, '0')
	   };
   };
/**
 * @desc 判断大于0的正整数，
...
export const isNoValueLevel = (arr) => {
    ...
    return !!(validate['A'].length || validate['B'].length || validate['C'].length);
};
