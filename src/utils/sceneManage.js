...
export function setScene({ scene }) {
  ...
  if (!sceneManage[SCENE_QUEUE_KEY]) sceneManage[SCENE_QUEUE_KEY] = [senceObj];
    ...
    sceneManage[SCENE_QUEUE_KEY] = sceneManage[SCENE_QUEUE_KEY].push(senceObj);
  // 初始化 场景记录
...
export async function fetchScene(render) {
  if (!sceneManage[SCENE_QUEUE_KEY] || sceneManage[SCENE_QUEUE_KEY].length == 0)
    return false;
  let senceObj = sceneManage[SCENE_QUEUE_KEY][0];
  if (!senceObj[IS_IN_SCENEIS_KEY]) return false;
  render =
    render ||
    (TYPE_MAP[senceObj[SCENE_PARSE_KEY].type] &&
      TYPE_MAP[senceObj[SCENE_PARSE_KEY].type].render);
