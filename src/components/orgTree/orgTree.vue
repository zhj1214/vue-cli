...
<template>
  <div class="view">
    ...
    <div>
      <div
        v-if="selecttArr.length < 4"
        class=" breadcrumb flex-row"
      >
        <div
        ...
        >
          {{ item.label }}
          <div style="margin: 0px 4px; color: #dcdee2">
      ...
      >
        <div
          class="breadcrumbItem"
          style="display: flex;"
          @click="dropdownClick(selecttArr[0].id)"
        >
...
export default {
  ...
  methods: {
    ...
    onSearchOrgRequest(param) {
      this.$ajaxPost("/user-server/platform/department/pageTree", param).then(
        (res) => {
