      ...
      <Row style="margin-bottom:24px;">
        <i-col span="16">
          <Cascader
            ...
            :filterable="true"
            placeholder="请选择所属组织"
            style="width: 140px;display: inline-block;text-align: left;margin-right: 20px"
  ...
  data(){
    return {
      ...
      cols:[
        ...
        {title:'商品品牌',key:'goodsBrandText',render:(h,params)=>{
          return <div>{params.row.goodsBrandText}（{params.row.brandId}）</div>
        }},
        {title: '所属组织', width:200,key: 'orgCompleteName',render:(h,params)=>{
            ...
            return <div>
                <Tooltip placement="top" max-width="300">
                    ...
                    <div>
                    <p style="text-align:left;font-size:15px;cursor: pointer;display: -webkit-box;-webkit-line-clamp: 1;-webkit-box-orient: vertical;text-overflow: ellipsis;overflow: hidden;">{ params.row.orgName}</p>
  ...
  methods:{
    ...
    async loadGoodsSpecies(){
      const { data } = await this.$ajaxPost(
        "/goods-plus-server/v2/api/goods/spu/detail",
        {
          id: this.curGoods.id,
          loadSku: "true",
          loadBase: "true",
    ...
    async loadGoodsList(){
      ...
      let obj = {
        ...
        search:this.search.search,pageNum:this.search.page,pageSize:this.search.pageSize,source:this.checkedOrgId,
    ...
    async loadCategoryData () {
       const {code,data} = await this.$ajaxPost('/goods-plus-server/v2/api/goods/category/list',{
          parentId: "",
          type: "1",
          needSpuCount: "true",
          needTreeRel: "true",
          ...
          arr &&
            arr.forEach((c) => {
              if (c.parentId === item.id) {
                ...
                if (!item.children.some((d) => d.id === c.id))
    ...
    categorySearch (value) {
      ...
      this.search.page = 1; this.search.pageSize = 10;
      this.loadGoodsList();
    }
  }
}
