      ...
      >
        ...
        <SelfTitle :title="defaultCols">
          ...
          >
            ...
            <div
              v-if="item.dMkey"
              style="display:flex;flex-direction:row;position:relative;left:-60px;min-width: 75%;"
            >
              ...
              <InputNumber
                ...
                :readonly="item.isreadonly?true:false"
                :value="item.value*1||null"
                style="width:48%;position:relative;top:12px"
                :max="item.max"
                :min="0"
  ...
  methods: {
    ...
    initTable(list, skus) {
      ...
      curList.forEach((item) => {
        ...
        let cSku = skus.filter(
          (sitem) =>
            sitem.specContentIds &&
            sitem.specContentIds.sort().join(",") === cId
        );
