<template>
  <view class="page-home">
    <view class="header-fix">
      <view class="swiper-wrap">
        <u-swiper
          ...
          :mode="swiperList.length === 1 ? 'none': 'round'"
          height="620"
          border-radius="0"
          :list="swiperList"
          @click="onSwiperTap"
      ...
      <view class="bar-wrap" :style="{paddingTop: statusBarHeight+'px',background:(opacity>2?'#ffffff': 'transparent')}">
        <view class="bar-wrap__bg" :style="'opacity:' + opacity + ';'"></view>
          <!-- 导航栏 -->
        <view class="nav-bar">
          ...
          <view class="nav-bar__title">恒太荟</view>
        </view>
        <!-- 搜索栏 -->
        <view class="search-bar">
          <view class="search-bar__field" @tap="onTapSearch" style="background:#F4F4F4;">
            <image
              class="search-bar__field--icon"
              mode="widthFix"
              src="/static/images/store/search/search.png"
            ></image>
            <view class="search-bar__field--input">搜索好物</view>
          </view>
          <view class="search-bar__action" @tap="onQrcodeTap">
            <image
              class="search-bar__action--icon"
              mode="widthFix"
              src="https://eos-wuxi-1.cmecloud.cn/crmoss/f757033f-4631-4a04-ab32-4d8d930df620.png"
            ></image>
          </view>
        </view>
      </view>
    ...
    <!-- grid -->
    <view v-if="grids.length" class="nav-card">
      <view
        v-for="(gItem, gIndex) in grids"
        :key="gIndex"
        class="nav-card__grid"
        @tap="onGridTap(gItem, gIndex)"
      >
        <image
          ...
          :src="gItem.image"
        ></image>
        <view class="nav-card__text">{{ gItem.title }}</view>
      </view>
    </view>
    <!-- 运营位 -->
    <view v-if="column2.length || column1.length" class="adv-card">
      <view class="adv-card__2">
        <view
          v-for="(gItem, gIndex) in column2"
          :key="gIndex"
          class="adv-card__2--item"
        >
          <image
            class="adv-card__2--img"
            mode="widthFix"
            :src="gItem.image"
            @tap="onColumnTap(gItem, gIndex)"
          ></image>
        </view>
      </view>
      <view
        v-for="(gItem, gIndex) in column1"
        :key="gIndex"
        class="adv-card__1"
      >
        <view class="adv-card__1--item">
          <image
            class="adv-card__1--img"
            mode="widthFix"
            :src="gItem.image"
            @tap="onColumnTap(gItem, gIndex)"
          ></image>
    ...
    <!-- 分组 -->
    <app-group-card v-if="groups.length>0" :groups="groups" @tap-more="onTapMore" @tap-goods="onGoodsCardTap"></app-group-card>  
    ...
    <view class="page-body">
      <u-tabs
        ...
        @change="onTabChange"
      ></u-tabs>
      <view v-if="curTabIndex === 2" class="page-body__list">
      ...
      <view v-if="curTabIndex === 0" class="page-body__list-c">
        <view v-for="item in list" :key="item.couponId" class="page-body__list-c--item">
          <app-coupon-card
            :card="item"
          ></app-coupon-card>
        </view>
      </view>
      <view v-if="curTabIndex === 1" class="page-body__list-v">
        <view v-for="item in list" :key="item.id"  class="page-body__list-v--item">
          <app-activity-card :card="item"></app-activity-card>
        </view>
      </view>
       <app-empty
        v-if="!list.length && params.page === 1" 
        :card="{
          img: 'https://eos-wuxi-1.cmecloud.cn/crmoss/98af2b56-7e99-409c-b422-b1ae19195a92.png',
          text: '暂无数据 ~',
        }"
       ></app-empty>
    </view>
...
import { adaptGroups, adaptChangeMall, adaptGoodsList, adaptCouponList, adaptActivityList } from '@/utils/adaptors/shop.js'
...
// }) 
  ...
  methods: {
     ...
     * */ 
    currentMallisDL(){
      if(uni.$localStorage.getItem("memberId") && !app.globalData.currentMallisDL)
                uni.$api.apiRequest("getMallInfo", {
    ...
    async jumpLogic() {
      ...
      if (!mallInfo.orgid && self.refuseMallLocation == undefined && self.isRefuseMallLocation == undefined) {
