      ...
      <view class="header">
        ...
        >
          <image
            ...
            mode=""
          ></image>
          <view class="integralText" style="margin-left: 4px;"
            >{{ ponitName }}规则</view
          >
        </view>
        <view class="integralText">账户总{{ ponitName }}</view>
        <view class="integralValue">{{ totalIntegral || 0 }}</view>
      </view>
      <!-- 积分列表 -->
      <view class="content">
        ...
        <view class="">
          <view class="item" v-for="(item, index) in currentData" :key="index">
            ...
            <view
              class="item-value"
              :style="!item.isRed ? 'color:#444444;' : 'color:#FE5D4D;'"
    ...
    getListData() {
      ...
      uni.$api.apiRequest("pointsDetail", param).then((res) => {
        if (res.code == 10000 && res.data) {
		  ...
		  this.totalIntegral = res.data.pointsScore
        }
      });
    },
    change(index) {
    ...
    jumpRelus() {
      ...
      uni.navigateTo({
        url:
          ...
          `${this.ponitName}规则&url=pointsRelus`,
      });
    },
  },
};
