 ...
 * @LastEditors: zhj1214
 * @LastEditTime: 2021-04-28 16:14:49
    ...
    <view class="header">
      ...
      <view class="list">
          ...
          >
            <CouponCard :card="item">
              <view slot="right">
                ...
                >
                  <template>{{ `${item.status != 2 ? '免费领取' : '已抢光'}` }}</template>
                </u-button>
              </view>
            </CouponCard>
          </view>
        ...
        <view v-else class="nodata">
          <app-empty
            :card="{
              img: 'https://eos-wuxi-1.cmecloud.cn/crmoss/6bb11724-c450-456c-8e3b-c5cf1d789814.png',
              text: '暂无优惠券',
            }"
          >
          </app-empty>
...
<script>
  ...
  export default {
    ...
    onShow() {
      if (!uni.$localStorage.getItem('Token')) {
        ...
        if (!uni.$localStorage.getItem('currentMall')) {
          this.currentMallInfo(app.globalData.orgId || uni.$localStorage.getItem('orgId'))
        } else {
          // 已登录 是否需要切换组织
          const orgId = this.sceneParse(this.options.scene || '').orgId
          if (orgId && uni.$localStorage.getItem('orgId') != orgId) {
            this.currentMallInfo(app.globalData.orgId || uni.$localStorage.getItem('orgId'), true)
          } else {
            this.getPageList()
          }
        }
    ...
    onLoad(options) {
      ...
      if (!orgId) {
        const list = this.sceneParse(options.scene || '')
        orgId = list.orgId
