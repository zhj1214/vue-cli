<template>
  <li class="single-attr">
    ...
    >
      <OptionGroup
        v-for="ops in filterAttrsList"
        :key="ops.code"
        :label="ops.value"
      >
        <Option
          v-for="op in ops.children||[]"
          :key="op.code"
          :value="op.code"
        >
          {{ op.value }}
    ...
    >
      <Option
        v-for="op in ob.supportOperators||[]"
        :key="op.code"
        :value="op.code"
      >
        {{
    ...
    <InputNumber
      v-if="rule.operator!=='between'&&ob.elementType==='Input'&&(ob.format==='float'||ob.format==='int')"
...
export default {
    ...
    computed: {
        ...
        sceneList() {
            ...
            // 登录组织、最近登录组织筛选
            let loginOrg =  eventType === "last_login_org" ||
                eventType === 'login_org' ||
                        ...
                        if (data && data[0]) {
                            ...
                            if (data[0].children && data[0].children[0]) {
                                ...
                                if (data[0].children[0].children && data[0].children[0].children[0]) {
                                    ...
                                    if (data[0].children[0].children[0].children && data[0].children[0].children[0].children[0]) {
