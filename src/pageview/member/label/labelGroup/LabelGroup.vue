...
export default {
    ...
    methods: {
        ...
        createLabelType() {
            ...
            this.$ajaxPost('/label-server/labelType/add', {name: this.labelObj.name})
                .then(res => {
                    if (res.code === 10000) {
        ...
        updateLabelType() {
            this.commitLoading = true;
            this.$ajaxPost('/label-server/labelType/update', this.labelObj)
                .then(res => {
                    if (res.code === 10000) {
                        this.$Message.success(res.message || '操作成功');
                        this.editLabelStatus = false;
        ...
        deleteLabelType(id) {
            this.$ajaxPost('/label-server/labelType/delete', {id})
                .then(res => {
                    if (res.code === 10000) {
                        this.$Message.success(res.message || '操作成功');
                        this.onSearch();
