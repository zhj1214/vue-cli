...
<template>
  <div class="complete-box padding-box">
    <p>
      <img
        :src="complete.isComplete?AuthSuccessIcon:AuthFailIcon"
        alt="授权结果"
      >
    </p>
    <h3>{{ complete.isComplete?'恭喜！授权成功':'抱歉，授权绑定失败' }}</h3>
    <p class="complete-tip">
      {{ complete.isComplete?`${cType=='account'?'公众号':'小程序'}「${complete.account}」已完成授权`:matchText[complete.status] }}
    </p>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        AuthFailIcon,AuthSuccessIcon,
        // 1.授权成功 2 没有开发平台账户管理权限 3 已授权其他商铺 4 个人小程序 5 获取授权信息失败
        matchText:{
          2:`未授权所有权限可能导致${this.cType=='account'?'公众号':'小程序'}不可用，请重新授权并勾选所有权限`,
