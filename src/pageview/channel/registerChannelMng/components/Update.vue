<template>
  ...
  >
    ...
    <FormItem label="注册渠道归属方">
      {{
        detail.affiliationOrganizationName
          ? detail.affiliationOrganizationName
          : '--'
      }}
    </FormItem>
...
export default {
  ...
  methods: {
    ...
    fetchDetail(id) {
      this.$ajaxGet('/member-server/api/channel/info', { channelId: id }).then(
        (res) => {
        ...
        if (valid) {
          const params =
            this.detail.type === 1
              ? {
                  ...
                  affiliationOrganizationId: this.detail
          ...
          if (this.detail.type === 2) {
            const isRepeat = await this.validatRepeat(
             { channelId: this.detail.id, orgName: params.orgName }
            );
            if (isRepeat) {
