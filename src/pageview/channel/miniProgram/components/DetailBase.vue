<template>
  <div class="detail-base">
    ...
    >
      ...
      />
      <span style="color: #171725"> {{ versionData.editionInfo.lastUpdateTime }} -  {{ versionData.editionInfo.updateStateDesc }}，请耐心等待</span>
    </div>
    <!--可升级 -->
    <div
    ...
    >
      <i
        style="color: #756CEA"
        class="icon iconfont iconInfo"
      />
      <span style="color: #171725"> 已检测到小程序新版本{{ versionData.editionInfo.onlineVersionCode }},您可手动升级到最新版本 </span>
      <span
      ...
      >
        <span
          style="position:relative;z-index: 1;color: #B5B5BE;cursor:pointer;"
        >升级</span>
      </Tooltip>
    </div>
    ...
    >
      <i
        style="color: #756CEA;"
        class="icon iconfont iconError"
      />
      <span style="color: #171725"> {{ versionData.editionInfo.lastUpdateTime }} -
    ...
    <div class="version-info clear">
      <div class="left-img">
        <img
          :src="appDetail.headImg||defaultAppLogo"
          alt=""
        >
      </div>
      <div class="right-info">
        <h3 style="font-size: 16px;margin-bottom: 24px;">
          {{ appDetail.nickName }}
        </h3>
            ...
            >
              ...
              <span>|
                <span
                  ...
                  @click="onDisabled"
                >
                  {{ versionData.usageState==='1'?'禁用':(versionData.usageState==='2'?'启用':'') }}
                </span>
                <Tooltip
                  :max-width="200"
                  :content="versionData.usageState==='1'?'禁用小程序，您的小程序将显示「该商户已关闭」，导致您的用户无法正常访问；禁用后若再次重新使用，小程序数据将被保留。':'恢复正常使用小程序'"
                >
                  <i
        ...
        <!--完善信息-->
        <div
          v-if="versionData.versionState==='3'"
          class="clear"
        >
      ...
      <div class="base-block">
        <div class="block">
          ...
          >
            <div
              class="clear"
              style="line-height: 24px;color: #171725"
            >
              已授权
              <span
        ...
        >
          ...
          <div class="right">
            <div
              ...
              style="color: #171725"
            >
              <span>{{ isRelateAccount?'已':'可' }}关联公众号 「{{ relateAccount.appName||'' }}」</span>
              <Poptip
              ...
              <span
                ...
                class="span-a"
                style="float: right"
                @click="linkWeappToAccount"
              >{{ isRelateAccount?'解除关联':'关联' }}</span>
            </div>
      ...
      >
        ...
        >
          <div class="cert-upload">
            ...
            <p class="p-text">
              最多上传一个文件，请上传pkcs12格式证书 (apiclient_cert.p12)
            </p>
          </div>
        </FormItem>
        <FormItem label="支付授权目录填写说明：">
          ...
          <p
            v-for="(item,index) in payData.authList"
            :key="index"
            class="span-a"
          >
            {{ item }}
    ...
    >
      ...
      />
      <div slot="footer">
        <Button @click="authListModal=false">
          关闭
        </Button>
      </div>
    </Modal>
    ...
    >
      <div>
        ...
        >
          <i
            ...
            class="icon iconfont iconInfo"
          />
          <span style="color: #171725">解除小程序授权后，如果有已经在运行的小程序将无法再继续使用，请谨慎操作。</span>
        </div>
      ...
      <div slot="footer">
        ...
        <Button
          type="primary"
          :loading="isCancelBinding"
          @click="onCommitCancelBind"
        >
          解除绑定
        </Button>
      </div>
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				payDataValid: {
					...
					merchID: [
						{
							required: true, message: '请输入微信商户号', trigger: 'change'
						}
					],
					// 秘钥
					key: [
		...
		computed: {
			authUrl() {
				return 'https://mp.weixin.qq.com/cgi-bin/componentloginpage?component_appid=' + this.appData.componentAppid + '&auth_type=2&pre_auth_code=' + this.appData.preAuthCode + '&redirect_uri=' + this.redirectUrl;
			},
			isCheckDisabled() {
		...
		methods: {
			...
			async checkRelateMenu(){
				const {data} = await this.$ajaxGet('/wechatplatform-server/api/mx/mp/menu/checkLinkWeapp',{appId:this.appDetail.authorizerAppid})
				this.isRealteMenu = data;		// true 关联 false 没有关联
			},
			// 根据小程序原始ID校验是否关联了公众号
			async checkRelateAccount(){
				const {data} = await this.$ajaxGet('/wechatplatform-server/api/edition/wx/app/link/status',{userName:this.appDetail.userName});
				if(data!=null){
					// 状态 0未关联 1关联 2关联中
			...
			async getRelateAccount(){
				// appType 1公众号 2小程序
				const {data} = await this.$ajaxGet("/wechatplatform-server/api/edition/wx/platform/app",{appType:1});
				if(data){
			...
			createReAuth() {
				this.$ajaxGet('/wechatplatform-server/api/auth/preAuthCode')
					.then(res => {
						if (res && res.code === 10000) {
							const {componentAppid, preAuthCode} = res.data || {};
							this.$nextTick(() => {
								const href = 'https://mp.weixin.qq.com/cgi-bin/componentloginpage?component_appid=' + componentAppid + '&auth_type=2&pre_auth_code=' + preAuthCode + '&redirect_uri=' + this.redirectUrl;
			...
			sendValidCode() {
				this.isSending = true;
				this.$ajaxGet('/wechatplatform-server/api/auth/sendSmsCode')
					.then(res => {
			...
			// 禁止、启用小程序
			onDisabled() {
				const type = this.versionData.usageState === '1' ? '2' : (this.versionData.usageState === '2' ? '1' : '');
				if (!type) return;
			...
			* */
			...
			updatePaySetting() {
				...
				this.$ajaxPost('/wechatplatform-server/api/auth/pay/setPaySet', {
					orgId: this.originOrgId,
					type: '2',
					merchName, merchID, key, certPassword, certUrl
				})
					.then(res => {
						if (res && res.code === 10000) {
							this.$Message.success(res.message || '操作成功');
