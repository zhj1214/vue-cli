...
<template>
  <div>
    <section class="top-desc padding-box">
      <div>
        <img
          :src="authData['headImg']||NosignAccountPic"
          alt="默认图片"
        >
        <aside>
          <h3>{{ authData['nickName']||'-' }}</h3>
          <p v-if="authData['alias']">
            公众微信号：{{ authData['alias'] }}
          </p>
          <p v-else>
            你的公众号暂未设置微信号，可设置并重新授权
          </p>
        </aside>
      </div>
      <a
    ...
    >
      ...
      <FormItem label="公众号授权状态：">
        <div class="infor-formitem">
          <aside>
            <p>已授权</p>
            <p>您的商户已获得该公众号的所有接口权限，可以正常对接微信；如果使用中发现接口异常，请重新授权</p>
          </aside>
          <aside>
            <a :href="authUrl">重新授权</a> <Divider type="vertical" />
            <a
              href="###"
              @click.stop="openRealseBind"
            >解除绑定</a>
          </aside>
        </div>
      </FormItem>
      ...
      <FormItem label="主体信息：">
        <div class="infor-formitem">
          <aside><p>{{ accoutSetting.principalName || '-' }}</p></aside>
        </div>
      </FormItem>
      <FormItem label="关联小程序：">
        <div class="infor-formitem">
          ...
          <aside>
            <a
              href="https://mp.weixin.qq.com/?token=&lang=zh_CN"
              target="_blank"
            >关联设置</a> 
          </aside>
        </div>
    ...
    >
      ...
      > 
        ...
        >
          ...
          />
          <Button
            style="margin-left:8px;color:#696974"
            @click="sendValidCode"
          >
        ...
        <p
          ...
          class="auth-check"
        >
          <Checkbox v-model="bindInfo.agree">
            已知晓解除绑定的风险，确认解绑
          </Checkbox>
        </p>
        <footer style="text-align:right; padding-top:12px;border-top:solid 1px #f0f0f0;">
          <Button
    ...
    >
      ...
      <p style="text-align:right;">
        <Button @click="authVisible=false">
          关闭
        </Button>
      </p>
    </Modal>
...
export default {
  ...
  computed: {
    // 绑定授权链接
    authUrl() {
      return `https://mp.weixin.qq.com/cgi-bin/componentloginpage?component_appid=${this.preAuth['componentAppid']}&auth_type=1&pre_auth_code=${this.preAuth['preAuthCode']}&redirect_uri=${location.origin+'/channel/officeAccount/setting'}`;
    },
    // 授权表格
    authCol() {
      return [
        {key:"limitName",title:"权限项"},
        {
          ...
          render(h,params) {
            return h('Icon',{
              props:{
                type:params.row.hasAuth?'ios-checkmark-circle-outline':'ios-close-circle-outline',
                size:'20',
  ...
  methods: {
    sendValidCode() {
      this.isSending = true;
      this.$ajaxGet(this.API.SEND_VALID_CODE)
        .then(res => {
    ...
    // 获取公众号授权权限列表
    async viewAuthList() {
      const {data} = await this.$ajaxGet(this.API.AUTH_LIST,{authorizerAppid:this.accoutSetting['authorizerAppid']});
      if(data){
        this.authList = data;
...
<style lang="scss">
  ...
  .auth-list-table {
    .ivu-table-body{
      max-height: 480px; overflow: hidden; overflow-y: auto;
    }
  }
</style>
<style lang="scss" scoped>
  .top-desc{
    ...
    >div{
      ...
      img{
        margin-right: 24px;height: 56px; width: 56px; border-radius: 50%;
      }
    }
  ...
  .infor-formitem{
    display: flex; justify-content: space-between;
    .ivu-divider{min-height:14px;height: 0.6em;}
    p{
      font-size:12px;
      font-family:SFUIDisplay-Light,SFUIDisplay;
      font-weight:300;
    ...
    p:first-child{
      font-size:14px; color:#171725;
    }
    padding-bottom: 20px; border-bottom: solid 1px #E2E2E9;
  }
  .account-form{
    h4{
      height:38px; margin-bottom: 40px;
  ...
  .auth-title{
    ...
    margin-bottom: 20px;
    .ivu-icon {color:#FF951B; font-size: 16px; margin-right: 8px; font-weight: bold;}
  }
  .bind-box{
    section.mes-code{
  ...
  .auth-desc{
    color:#92929D;font-size:12px;
    text-indent: 8em;
    margin:8px 0 20px;
  }
  .auth-check{
    padding-left: 95px;margin-bottom:40px;
    label{
      font-size:14px;
      font-family:SFUIDisplay-Light,SFUIDisplay;
      font-weight:300;
