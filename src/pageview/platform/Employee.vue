    ...
    >
      <div class="ivu-modal-body">
        ...
        >
          ...
          >
            <div
              v-for="(item, oindex) in employeeData.orgRoleList"
              :key="oindex"
            >
              <div v-show="item.type != 2">
                ...
                >
                  <FormItem
                    :prop="'orgRoleList.' + oindex + '.departmentIdStr'"
                  >
                    ...
                    <div style="display: flex; align-items: center">
                      ...
                      <belong
                        ...
                        :show-current="true"
                        @selectOrg="
                          (value, arr) => casChange(value, arr, oindex)
                        "
                      />
                      <div v-else>
                        <orgSelect
                  ...
                  <a
                    v-show="oindex"
                    style="margin-left: -20px;position: relative;left: 15px;"
  ...
  methods: {
    ...
    formatsss(labels, selectedData, num) {
      ...
      if (selectedData.length) {
        this.employeeData.orgRoleList[num].orgId =
          selectedData[selectedData.length - 1].orgId;
      }
      return labels[index];
    },
    casChange: _.debounce(function(value, arr, idx) {
      ...
      this.employeeData.orgRoleList[idx].departmentId =[arr[arr.length - 1].value]||[arr[arr.length - 1].id];
    ...
    onBlurCheckPhone() {
      ...
      this.$ajaxPost("/user-server/platform/employee/checkPhoneExist", {
        ...
        console.log('orgRoleList.lengthorgRoleList.lengthorgRoleList.lengthorgRoleList.lengthorgRoleList.length')
        if (res && res.code === 10000) {
          ...
          if (res.data.isPhoneExit) {
            ...
            if (res.data.orgRoleList.length) {
              res.data.orgRoleList.map((item,idx) => {
                ...
                item.isHX = true;
                this.treeTempList.splice(idx, 0, item.parentDepartmentIds[item.parentDepartmentIds.length - 1]);
              });
            ...
            this.employeeData = {
              ...
              password: "",
              passwordCheck: "",
              orgRoleList: this.employeeData.orgRoleList.length?this.employeeData.orgRoleList:[
                {
                  departmentId: [],
                  departmentIdStr: localStorage.getItem("departmentId"),
                  orgId: "",
    ...
    deleteEmployee(ids) {
      ...
      this.$ajaxPost("/user-server/platform/employee/delete", ids).then(
        (res) => {
          if (res && res.code === 10000) {
            // 更新左边树 和 右边的表
            this.$eventEmitter.trigger("orgTreelist", {
    ...
    onOrgSelect(val,index) {
      console.log(val,'___',index,'______',this.departmentId,'__this.treeTempList__',this.treeTempList);
