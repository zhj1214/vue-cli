<template>
  <div class="addApproval-page">
    ...
    <div class="add-content">
      <div class="approval-jd"> 审批人 </div>
      <Row :gutter="64" style="padding-top: 24px;">
        <i-col v-for="(item, index) in approvalIds" :key="index" span="4">
          <div class="approve-people" style="position: relative; min-height: 90px;">
            <div style="max-width: 130px;" @click="openDetail(item)">
          ...
          <Modal
            ...
            @on-ok="closeDetail"
          >
            <Table :columns="approvalColumns" :data="approvalList" height="200">
    ...
    <div class="add-content" style="margin-top: 24px;">
      <div class="top-notice">
        <img
          src="../../../assets/images/icon-info.png"
          alt=""
          style="position: absolute; top: 12px; left: 12px; width: 24px; height: 24px;"
        />
        <span>注：当所有审批角色对应的审批人为空时，审批自动通过。</span>
      </div>
      <LeftTreeLayout style="position: relative;">
        <template v-slot:left>
          <orgTree @select-change="treeChange" />
        </template>
          ...
          <div class="content">
            <div class="show-table">
              ...
              <Table :columns="columns" :data="tableList">
                ...
                <template v-slot:employeeCount="row">
                  <span>{{ row.employeeCount }}</span>
                </template>
                <template v-slot:action="row">
                  <Button class="action-text" @click="addApproval(row)"> 添加 </Button>
                </template>
              </Table>
            </div>
          </div>
          <div>
            <div class="footer" style="position: absolute; right: 0; margin-top: 24px;">
              <Button style="margin-right: 10px;" @click="cancelApproval()"> 取消 </Button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //编辑审批人
      editApproval(ids) {
        this.$ajaxPost('/user-server/platform/role/batchDetail', ids).then((res) => {
      ...
      addApproval(row) {
        ...
        var self = this
        self
          .$ajaxGet('/user-server/platform/role/detail', {
            roleId: row.id,
          })
          .then((res) => {
            if (res && res.code === 10000) {
      ...
      cancelApproval() {
        if (this.clickAdd) {
          this.approvalPeople.list = this.approvalPeople.list.splice(
            this.approvalPeople.list.length - 1,
            1
          )
        }
        this.$router.push({
