...
const timeFn = (h, params) => {
	return h('span', dateParser(Number(params.row.operateTime || params.row.createTime), 'YYYY-MM-DD HH:mm:ss'));
};
...
export function organizationTableCol() {
	return [
		...
		{
			title: '组织名称/id', key: 'name', minWidth: 130, render: (h, params) => {
				return h('div', [
		...
		{
			title: '类型', key: 'type', minWidth: 200,
			filters: [
				{ label: '部门', value: '3' },
				{ label: '区域', value: '4' },
				{ label: '商场', value: '5' },
		...
		{
			title: '所属组织', key: 'name', minWidth: 130,
			render: (h, params) => {
				return h('div', [
		...
		{
			title: '创建人', key: 'creatorName', minWidth: 130, render: (h, params) => {
				return h('div', [
		...
		{
			title: '操作', key: '', minWidth: 80, render: (h, params) => {
				return h('div', [
					this.pageAction.includes('update') ? (params.row.source !== 'ERP' ? h('span', {
						class: 'mr-5 span-a',
						on: {
							click: () => {
						...
						: '',
					this.pageAction.includes('delete') ? (params.row.canBeDelete ? h('Poptip', {
						props: {
							confirm: true,
							wordWrap: true,
							width: 210,
...
export function roleTableCol() {
	return [
		...
		{ title: "角色名称", key: "name" },
		{
			title: "所属组织", key: "belongOrg", render: (h, params) => {
				return h('div', [
		...
		{
			title: "创建人", key: "createName", render: (h, params) => {
				return h('div', [
		...
		{ title: "创建时间", key: "createTime", render: renderTime },
		{
			title: "操作", render: (h, params) => {
				return h('div', [
					this.pageAction.includes('update') ? h('span', {
						class: 'mr-5 span-a',
						on: {
							click: () => {
								this.isEditStatus = true;
					...
					this.pageAction.includes('delete') ? h('Poptip', {
						props: {
							confirm: true,
							wordWrap: true,
							width: 210,
...
export function employeeTableCol() {
	return [
		...
		{
			title: "匹配角色", key: "roleName", render: (h, params) => {
		...
		{
			title: "创建人", key: "creatorName", render: (h, params) => {
				return h('div', [
					h('p', { style: { color: "#171725", fontSize: "14px" } }, params.row.creatorName),
					h('p', { class: "p-text", style: { fontSize: "12px" } }, params.row.creatorPhone)
				]);
		...
		{ title: "创建时间", key: "createTime", render: renderTime },
		{
			...
			render: (h, params) => {
				return h('span', { class: params.row.isEnable ? 'is-enable' : 'is-disabled' }, params.row.isEnable ? '有效' : '无效');
			}
		},
		{
			title: "操作", render: (h, params) => {
				return h('div', [
					...
					// 	}
					// }, '编辑') : "",
					(!params.row.isAdmin && this.pageAction.includes('update')) ? (params.row.source !== 'ERP' ? h('span', {
						class: 'mr-5 span-a',
						on: {
							click: () => {
								this.isEditStatus = true;
					...
					!params.row.isAdmin && this.pageAction.includes('delete') ? (params.row.source !== 'ERP' ? h('Poptip', {
						props: {
							confirm: true,
							wordWrap: true,
							width: 210,
...
export function installTableCol() {
	return [
		// {type: 'selection', width: 80},
		{
			title: '审批流程名称', key: 'approveTradeName', minWidth: 130, render: (h, params) => {
				return h('div', [
					h('p', { style: { color: "#171725", fontSize: "14px" } }, params.row.name),
					h('p', { class: "p-text", style: { fontSize: "12px" } }, params.row.orgId)
				]);
		...
		{
			title: '状态', key: 'type', minWidth: 200,
			filters: [
				{ label: '已启用', value: '1' },
				{ label: '已停用', value: '-1' },
			],
		...
		{
			title: '操作', key: 'action', minWidth: 80, render: (h, params) => {
				return h('div', [
