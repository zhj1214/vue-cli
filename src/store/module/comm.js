  ...
  actions: {
    ...
    loadCouponList({ state }, search) {
      ...
      let iSearch = JSON.parse(
        JSON.stringify(
          { ...search, orgId: localStorage.orgId } || state.couponSearch
        )
      );
    ...
    async loadAllBrands({ state }) {
      console.log(state);
      return await axios_post("/goods-plus-server/v2/api/goods/brand/list",{pageNum: 1,pageSize: 99999,source: localStorage.getItem("orgId")});
    },
    async getCouponDetail({ state }, id) {
      console.log(state);
      return await axios_post(
        "/coupon-server/api/coupon/contact/getCouponDetail",
        { couponId: id }
      );
    },
