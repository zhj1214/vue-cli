...
<template>
  <Layout>
    ...
    >
      ...
      <div
        v-show="curMenu.length"
        style="margin-top:72px;position:relative"
      >
        ...
        >
          ...
          >
            <h3
              :class="[{ isActive: items.actived }]"
              @click="toggleMenu(indexS)"
            >
              <span><i
                v-if="items.icon"
                :class="'icon iconfont ' + items.icon || ''"
              />{{ items.meta ? items.meta.title : "" }}</span>
            </h3>
            <div
    ...
    <Layout>
      <Header>
        ...
        >
          <span
            class="orgBox span-a"
          ><i class="icon iconfont iconCompany" />{{ orgName }}</span>
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    <i
                      @mouseenter="mouseenterIcon(item.id)"
            ...
            >
              <span>
                <i
                  class="avatar"
                  :style="
                    `background:url(${$store.state.headImg ||
                      defaultAvatar})no-repeat center;background-size: cover;`
                  "
                />
      ...
      >
        ...
        >
          <h2
            v-show="$route.name !== 'people-detail' && !$route.meta.h2"
            class="h2"
          >
  ...
  computed: {
    ...
    curMenu: {
      get() {
        if (!this.curMenuCopy) {
          const list =
            this.$store.getters.menuList.length &&
      ...
      this.$ajaxGet("/user-server/auth/switchOrg", {
        ...
        .then(res => {
          if (res && res.code === 10000) {
            ...
            localStorage.setItem(
              "departmentName",
              res.data.userInfo.departmentName || ""
            );
