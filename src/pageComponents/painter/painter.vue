...
export default {
  ...
  watch: {
    ...
    disableAction: function (isDisabled) {
      this.outterDisabled = isDisabled;
      this.isDisabled = isDisabled;
    },
    clearActionBox: function (needClear) {
  ...
  methods: {
    ...
    doAction(action, callback, isMoving, overwrite) {
      ...
      if (newVal && newVal.url && doView.url && newVal.url !== doView.url) {
        downloader.download(newVal.url, this.LRU).then(path => {
    ...
    downloadImages(palette) {
      return new Promise((resolve, reject) => {
        ...
        if (paletteCopy.background) {
          preCount++;
          downloader.download(paletteCopy.background, this.LRU).then(path => {
        ...
        if (paletteCopy.views) {
          for (const view of paletteCopy.views) {
            if (view && view.type === 'image' && view.url) {
              ...
              downloader.download(view.url, this.LRU).then(path => {
