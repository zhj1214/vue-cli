export default {
    ...
    methods:{
        ...
        async getBatch (isSearchDirectOrgInfo = false) {
             const {data=[]} = await this.$ajaxPost('/user-server/internal/organization/batch',[localStorage.getItem('orgId')])
            if (data && data.length) {
        ...
        async searchDirectOrgInfo () {
            const {data=[]} = await this.$ajaxPost('/user-server/internal/organization/searchDepartment',
            {
